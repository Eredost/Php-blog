<aside class="blog__sidebar">
    ...
    <form action="<?= $templateVars['router']->generateUrl('articleList') ?>" class="search-bar" method="get">
    ...
    <div>
        ...
        <?php foreach($templateVars['lastPosts'] as $post): ?>
            ...
            <div class="article-aside">
                <a class="article-aside__image" href="<?= $templateVars['router']->generateUrl('articleShow', ['postId' => $post->getId()]) ?>">
                    <img src="<?= $templateVars['request']->baseURI() . $post->getImage() ?>" alt="">
                </a>
                <div class="article-aside__head">
                    <h3 class="article-aside__title">
                        <a href="<?= $templateVars['router']->generateUrl('articleShow', ['postId' => $post->getId()]) ?>"><?= $post->getTitle() ?></a>
                    </h3>
